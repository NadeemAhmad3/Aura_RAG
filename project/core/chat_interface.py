...
SYSTEM_NODE_CONFIG = {
    ...
    "orchestrator":           {"title": "🧠 Agent: Planning & Reasoning"},
    "agent:orchestrator":     {"title": "🧠 Agent: Planning & Reasoning"},
    "compress_context":       {"title": "🗜️ Agent: Context Compression"},
    "agent:compress_context": {"title": "🗜️ Agent: Context Compression"},
}
...
class ChatInterface:
    ...
    def chat(self, message, history, user_id=None, session_id=None):
        ...
        try:
            ...
            for chunk, metadata in self.rag_system.agent_graph.stream(stream_input, config=config, stream_mode="messages"):
                ...
                elif isinstance(chunk, AIMessageChunk) and chunk.content and node not in SYSTEM_NODES and node not in SILENT_NODES:
                    self._handle_llm_token(chunk, node, response_messages)

                yield response_messages

            # Save the updated history to MongoDB Atlas
            if user_id and session_id:
                ...
                if mongo.enabled:
                    ...
                    current_user_message = {"role": "user", "content": message.strip()}
                    full_messages = formatted_history + [current_user_message] + response_messages
                    mongo.save_session_messages(session_id, user_id, full_messages)
